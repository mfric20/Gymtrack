...
export default async function handler(
  ...
  if (req.method === "POST") {
    try {
      if (session) {
        console.info("API zahtjev za ulogu djelatnika u teretani!");
      ...
      console.error(
        `Greška kod API zahtjeva za ulogu djelatnika u teretani | Poruka ${error}`
      );
      return res.status(400).json({
        error: `Greška kod API zahtjeva za ulugu djelatnika u teretani | Poruka ${error}`,
      });
    }
  }
}
